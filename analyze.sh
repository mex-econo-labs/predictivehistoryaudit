...
- STATUS VALUES: untested | confirmed | partially_confirmed | disconfirmed | unfalsifiable | contested_unresolved
  * Use 'contested_unresolved' sparingly and only when ALL THREE conditions hold: (1) the claim cannot be confirmed from open sources, (2) the claim cannot be ruled out from open sources, (3) non-fringe actors are publicly asserting it. The purpose is to avoid the October Surprise failure mode — where absence of documentation silently becomes proof of the negative. For 'contested_unresolved' entries, the status_note must list the documented facts the claim rests on, who is asserting it, and what evidence would resolve it.
...
- VERDICT: populate 'strengths', 'weaknesses', 'viewer_advisory' as usual. ALSO populate the optional 'steelman' field when the lecture's specific claims and reasoning fail but the underlying concern is legitimate, shared by mainstream scholars, or survives charitable reading. Write it for a viewer who heard the lecture and senses something real beneath the flawed argument — give them a rigorous frame for that intuition. Omit steelman (set null) only when the lecture's concerns do not survive steelmanning.
